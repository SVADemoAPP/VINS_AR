...
void ViewController::getRefinePath(std::vector<std::vector<float >> &path) {
    std::vector<Vector3f>::iterator path_iter = keyframe_database.refine_path.begin();
    for (; path_iter != keyframe_database.refine_path.end(); path_iter++) {
        std::vector<float> pos;
        pos.push_back((*path_iter)[0]);
        pos.push_back((*path_iter)[1]);
        pos.push_back((*path_iter)[2]);
        path.push_back(pos);
    }
}
